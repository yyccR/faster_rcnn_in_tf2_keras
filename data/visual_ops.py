...
def draw_bounding_box(im, cls, scores, x_min, y_min, x_max, y_max, thickness=2, color=(11, 252, 3), txt_size=0.35):
    ...
    if cls and scores:
        ...
        if y_rect_min < 0:
            ...
            y_txt = int(y_max + 5)
        # draw text box
        cv2.rectangle(im, (x_rect_min, y_rect_min), (x_rect_max, y_rect_max), (11, 252, 3), -1)
        # draw txt
        cv2.putText(im, txt, (x_txt, y_txt), cv2.FONT_HERSHEY_SIMPLEX, txt_size, (0, 0, 0))
    return im


def draw_instance(im, masks, alpha=0.5):
    im_cp = im.copy()
    num_instance = np.shape(masks)[2]
    if num_instance > 0:
        colors = _random_colors(num_instance)
        for i in range(num_instance):
            color = colors[i]
            for c in range(3):
                im_cp[:, :, c] = np.where(masks[:, :, i] == 1,
                                          im_cp[:, :, c] *
