...
def proposal_target_layer(rpn_rois, rpn_scores, gt_boxes, num_classes, use_gt=False, train_batch_size=256,
                          fg_fraction=0.5, train_fg_thresh=0.5, train_bg_thresh_hi=0.5, train_bg_thresh_lo=0.1,
                          train_bbox_normalize_targets_precomputed=True,
                          train_bbox_normalize_means=(0.0, 0.0, 0.0, 0.0),
                          train_bbox_normalize_stds=(0.1, 0.1, 0.2, 0.2), bbox_inside_weight=(1.0, 1.0, 1.0, 1.0)):
    ...
    labels, rois, roi_scores, bbox_targets, bbox_inside_weights = _sample_rois(
        ...
        train_fg_thresh=train_fg_thresh,
        train_bg_thresh_hi=train_bg_thresh_hi,
        train_bg_thresh_lo=train_bg_thresh_lo,
        train_bbox_normalize_targets_precomputed=train_bbox_normalize_targets_precomputed,
        train_bbox_normalize_means=train_bbox_normalize_means,
        train_bbox_normalize_stds=train_bbox_normalize_stds,
...
def _get_bbox_regression_labels(bbox_target_data, num_classes, bbox_inside_weight):
    """Bounding-box regression targets (bbox_target_data) are stored in a
    compact form N x (class, tx, ty, tw, th)
    ...
    row_col_inds = tf.concat([tf.expand_dims(row_inds, 1), tf.expand_dims(col_inds, 1)], axis=1)
    updates_target_data = tf.reshape(tf.gather(bbox_target_data[:, 1:], inds), (-1,))
    updates_inside_weight = tf.reshape(tf.tile(bbox_inside_weight, [tf.shape(inds)[0]]), (-1,))
...
def _compute_targets(ex_rois, gt_rois, labels,
                     train_bbox_normalize_targets_precomputed,
                     train_bbox_normalize_means,
                     train_bbox_normalize_stds):
    """Compute bounding-box regression targets for an image."""

    targets = bbox_transform_tf(ex_rois, gt_rois)
    if train_bbox_normalize_targets_precomputed:
        # Optionally normalize targets by a precomputed mean and stdev
        targets = (targets - train_bbox_normalize_means) / train_bbox_normalize_stds
    labels_expand = tf.expand_dims(labels, axis=1)
    targets_add_labels = tf.concat([labels_expand, targets], axis=1)
    return targets_add_labels
...
def _sample_rois(all_rois, all_scores, gt_boxes, fg_rois_per_image, rois_per_image, num_classes,
                 ...
                 train_bbox_normalize_targets_precomputed, train_bbox_normalize_means,
                 train_bbox_normalize_stds, bbox_inside_weight):
    """Generate a random sample of RoIs comprising foreground and background
    examples.
    """
    # overlaps: (rois x gt_boxes)
    ...
    # Select foreground RoIs as those with >= FG_THRESH overlap
    fg_inds = tf.reshape(tf.where(max_overlaps >= train_fg_thresh), shape=(-1,))
    bg_inds = tf.reshape(tf.where((max_overlaps < train_bg_thresh_hi) & (max_overlaps >= train_bg_thresh_lo)),
                         shape=(-1,))
    ...
    final_max_overlaps = tf.keras.backend.max(final_overlaps, axis=1)
    final_bg_inds = tf.reshape(
        tf.where((final_max_overlaps < train_bg_thresh_hi) & (final_max_overlaps >= train_bg_thresh_lo)),
        shape=(-1,))
